...
def build_system_prompt(config, tactics=None):
    ...
    prompt = (
        ...
        "la crypto. Levier max: FX 30x, indices/or 20x, ACTIONS 5x, crypto 2x (le plus FAIBLE). "
        "Ne te rabats PAS sur la crypto par défaut juste parce qu'elle est ouverte 24/7.\n"
        "  • ACTIONS INDIVIDUELLES (CFD, 5x) = la classe la plus RICHE EN NEWS et en "
        "catalyseurs datés (résultats, guidance, upgrades, lancements). Quand la bourse est "
        "ouverte et qu'un catalyseur clair pousse un titre liquide (AAPL, NVDA, MSFT…), c'est "
        "souvent le pari le plus LISIBLE. CAVEAT: une action peut GAPPER (profit warning "
        "-20% overnight) par-dessus le stop → ne JAMAIS garder une action à travers sa "
        "publication de résultats sans réduire fortement la taille.\n"
        "  • Indices/FX/or = les vrais 'bold bets' à FORT levier quand un setup macro est net.\n"
        "Si les marchés pertinents sont FERMÉS et qu'aucun setup n'a une conviction VRAIMENT "
        "forte, ATTENDS (le cash est une position).\n"
        "DONNÉES CHIFFRÉES (vérité) : le champ `market_data` de l'état contient, pour une "
        "watchlist d'instruments liquides de toutes les classes, le momentum CALCULÉ "
        "(mom_1w/1m/3m en %), la volatilité JOURNALIÈRE réelle (vol_daily_%), le drawdown "
        "depuis le plus-haut et l'écart à la MM20 — tirés de vraies séries de prix. "
        "UTILISE CES CHIFFRES comme source de vérité : (1) DIRECTION = signe du momentum "
        "passé (long si mom_1m/mom_3m > 0, short si < 0) ; (2) TAILLE = inverse de la "
        "volatilité (plus vol_daily_% est haute, PLUS PETITE la position). Ne te fie PAS "
        "aux pourcentages lus dans les articles pour ça — la recherche web sert à expliquer "
        "le POURQUOI et à confirmer le catalyseur, pas à mesurer le momentum.\n"
        "PROCESSUS D'ANALYSE (obligatoire avant toute ouverture) : (a) lis `market_data` "
        "pour repérer les 2-3 instruments au meilleur momentum (classes différentes) ; "
        "(b) recherche web le CATALYSEUR et l'actualité du jour pour ces candidats ; "
        "(c) pour le meilleur, formule une thèse HAUSSIÈRE *et* le risque baissier "
        "(qu'est-ce qui invaliderait le trade ?) ; (d) ne l'ouvre que si le momentum chiffré "
        "ET le catalyseur convergent. Cite les chiffres de market_data + la source news "
        "dans le rationale.\n"
